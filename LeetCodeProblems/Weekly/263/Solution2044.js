...
subsetsUtil=function(nums, subset, idx, res){
    res.push(subset);
    for(let i=idx; i<nums.length; ++i){
        subset.push(nums[i]);
        subsetsUtil(nums, [...subset], i+1, res);
        subset.pop();
    }
    return;
}
...
var subsets=function(nums){
    var res=[];
    var subset=[];
    var n=Math.pow(2, nums.length);
    var idx=0;
    subsetsUtil(nums, subset, idx, res);
    return res;
}

var countMaxOrSubsets = function(nums) {
    var res=subsets(nums);
    var mp=new Array(1e6+1).fill(-1);
    var sz=res.length;
    for(let i=0; i<sz; ++i){
        let currXor=0;
        let sz_=res[i].length;
        for(let j=0; j<sz_; ++j){
            currXor|=res[i][j];
        }
        mp[currXor]++;
