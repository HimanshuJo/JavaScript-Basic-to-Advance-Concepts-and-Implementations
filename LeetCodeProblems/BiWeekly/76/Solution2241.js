...
var ATM = function() {
    this.NUM_DENOM=5;
    this.DENOM=[20, 50, 100, 200, 500];
    this.NOT_POSSIBLE=[-1];
    this.numBanknotesPerDenom=new Array(this.NUM_DENOM).fill(0);
};
...
ATM.prototype.deposit = function(banknotesCount) {
    for(let i=0; i<this.NUM_DENOM; ++i){
        this.numBanknotesPerDenom[i]+=banknotesCount[i];
    }
};
...
ATM.prototype.withdraw = function(amount) {
    const withdrawnNumBanknotesPerDenom=new Array(this.NUM_DENOM).fill(0);
    for(let i=this.NUM_DENOM-1; i>=0&&amount>0; --i){
        let numBankNotes=Math.min(Math.floor(amount/this.DENOM[i]), this.numBanknotesPerDenom[i]);
        amount-=numBankNotes*this.DENOM[i];
        withdrawnNumBanknotesPerDenom[i]=numBankNotes;
    }
    if(amount===0){
        for(let i=0; i<this.NUM_DENOM; ++i){
            this.numBanknotesPerDenom[i]-=withdrawnNumBanknotesPerDenom[i];
        }
        return withdrawnNumBanknotesPerDenom
    }
