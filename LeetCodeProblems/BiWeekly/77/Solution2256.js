...
var minimumAverageDifference = function(nums) {
    var sz=nums.length;
    var prefSum=new Array(sz).fill(0);
    var suffSum=new Array(sz).fill(0);
    prefSum[0]=nums[0], suffSum[sz-1]=nums[sz-1];
    for(let i=1; i<sz; ++i){
    ...
    var temp=Number.MAX_VALUE, ans=0;
    for(let i=0; i<sz; ++i){
        let frstT=i+1;
        let lastT=sz-frstT;
        let curval=Math.abs(Math.floor(prefSum[i]/frstT)-(i+1<=sz-1?Math.floor(suffSum[i+1]/lastT):0));
        if(curval<temp){
            temp=curval;
            ans=i;
        }
